...
gg_isi_periods <- lapply(all_eeg_df, function(x) {
  ggplot(
    ...
    annotate(
      ...
      label= paste0(
        "Clust: ",
        x %>% dplyr::filter(eeg_state == "sync") %>% select(clustered) %>% unique()
        ),
      col = "blue"
      )+
    annotate(
      ...
      y=35,
      label= paste0(
        "Clust: ", x %>% dplyr::filter(eeg_state == "desync") %>% select(clustered) %>% unique()
        ),
      col = "red"
      )+
...
```{r echo=FALSE, fig.width=10, message=FALSE, warning=FALSE}
  ...
  dplyr::filter(!grepl(pattern = "120", ID)) %>% # 01 and 04 are not clustered
...
csd_df <- readRDS(file = file.path("data", "sync_desync", "csd_df.rds")) %>% 
  lapply(function(x) { x %>% mutate(levels = 0, eeg_state = "desync")})

  
baseline_df <- readRDS(file = file.path("data", "sync_desync", "baseline_df.rds")) %>% 
  lapply(function(x) { x %>% mutate(levels = 1, eeg_state = "sync")})
...
```{r echo=FALSE, fig.width=10, message=FALSE, warning=FALSE}
TERMINALS %>% 
ggplot(mapping = aes(x = origin, y = d_dendrite)) +
  geom_half_violin(
    position = position_nudge(x = -.15),
    trim = F,
    side = "l",
