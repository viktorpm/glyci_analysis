...
psth_tibble <- left_join(psth_tibble, CELL_INFO %>% select(file = file_name, pinch, control))
...
psth_tibble %>% 
  ...
  geom_histogram(aes(fill = control),bins = 30) +
  geom_vline(xintercept = 0) +
  geom_vline(aes(xintercept = train_length)) +
  # geom_text(
  #   data = psth_tibble %>% select(No_Trains) %>%  unique(),
  #   aes(label = No_Trains)) +
  facet_wrap(train_length %>% round() ~ file, scales = "free",)



# psth_tibble <- CreatePSTH("cell11_05_right_IL_t1_2890_pinch_stim_int10") 
# 
# ap <- IL_stim_firing %>% 
#   dplyr::filter(
#     file_name == "cell11_05_right_IL_t1_2890_pinch_stim_int10", 
#     signal_type == "AP") %>% 
#   pull(signal_time)
# 
# 
# stim <- start_times$cell11
# 
# RTM_creator <- function(ap, stim) {
#     ap_vect <- ap %>% as.vector()
#     stim_vect <- stim %>% as.vector()
...
# 
# 
# psth_mat[psth_mat > -1 & psth_mat < 9] %>% hist(breaks = 30) 
...
APC <- lapply(RECORDINGS$animal_id %>% unique(), APCount) %>%
  bind_rows() %>%
  mutate(norm_No_APs_all = No_APs / max(No_APs)) %>%
  group_by(animal_id) %>%
  mutate(norm_No_APs_animal = No_APs / max(No_APs))
```
...
Response probability was calculated as the number of APs in the boxplot range (25th percentile - 1.5\*IQR, 75th percentile + 1.5\*IQR) * 100
...
cowplot::plot_grid(wavelets[[2]], eeg_sum_plot[[2]], nrow = 2, align = "v")
```
