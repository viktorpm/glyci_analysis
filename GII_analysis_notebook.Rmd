...
APC <- lapply(RECORDINGS$animal_id %>% unique(), APCount) %>% 
  ...
  mutate(norm_No_APs_animal_stim_freq = No_APs/max(No_APs))
```
...
### Number of APs after each stimulus in stimulus trains (50 ms window). Normalized by the maximum number of AP followed by a stimulus across animals.
...
ggplot(
  data = APC %>%
    ...
    dplyr::summarise(No_APs = sum(norm_No_APs_all)),
  mapping = aes(
    x = stim_number,
    y = No_APs,
