...
sync_reltimes <- lapply(all_eeg_df, function(x) {
  x %>%
    ...
    RTM_creator() %>%
    reshape2::melt() %>%
    dplyr::mutate(eeg_state = "sync", ID = x$ID %>% unique()) %>%
    transmute(ID = ID, time = value, eeg_state = eeg_state)
...
desync_reltimes <- lapply(all_eeg_df, function(x) {
  x %>%
    ...
    RTM_creator() %>%
    reshape2::melt() %>%
    dplyr::mutate(eeg_state = "desync", ID = x$ID %>% unique()) %>%
    transmute(ID = ID, time = value, eeg_state = eeg_state)
...
bind_rows(sync_reltimes, desync_reltimes) %>% 
  ...
  facet_grid(ID~eeg_state, scales = "free_y")
...
left_join(
  ...
  dplyr::filter(!grepl(pattern = "01|06", ID)) %>% # 01 and 04 are not clustered
  ...
  geom_signif(
    ...
    map_signif_level = TRUE,
    test = "wilcox.test",
    test.args = list(paired = T, alternative = "greater"),
    margin_top = .1,
    color = "darkgray"
  ) +
  ggpubr::stat_compare_means(method = "wilcox.test", paired = T, method.args = list(alternative = "greater"))
...
EEG_ds_df %>%
  ...
  dplyr::mutate(
    ...
    state_length_sum = sum(state_length %>% unique(), na.rm = T)
  ) %>%
  dplyr::group_by(ID, eeg_state) %>%
  summarise(state_length_sum = unique(state_length_sum), No_AP = unique(No_AP)) %>%
  dplyr::filter(!grepl(pattern = "120", ID)) %>%
  # 01 and 04 are not clustered
...
left_join(
  ...
  # dplyr::filter(!grepl(pattern = "31", animal)) %>%
  ggplot(aes(x = eeg_state, y = No_clusters / state_length_sum)) +
  geom_point(aes(col = animal), size = 3) +
  geom_line(aes(col = animal, group = animal)) +
  geom_boxplot(
    aes(x = eeg_state, y = No_clusters / state_length_sum),
    inherit.aes = F,
    alpha = 0, width = 0.2, lwd = 1, fatten = 1.2
