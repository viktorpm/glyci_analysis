...
IL_stim_firing %>% dplyr::group_by(signal_type) %>% slice(1:5)
```
...
IL_baseline_firing %>% dplyr::group_by(signal_type) %>% slice(1:5)
```
...
psth_tibble %>% 
  ...
  geom_histogram(
    data = . %>% 
      dplyr::filter(control == F,  train_length < 4),
    binwidth = 0.164,
    alpha = 0.3
  ) +
   geom_histogram(
    ...
    binwidth = 0.164,
    aes(fill = inhib_strength, col = gap)
    ) +
  geom_vline(xintercept = 0) +
  geom_vline(aes(xintercept = train_length)) +
  facet_wrap(file~train_length %>% round() , scales = "free") +
  theme(
...
psth_tibble_gap %>% 
  # dplyr::filter(cell_id %in% c("cell06","cell07","cell08","cell09")) %>%
