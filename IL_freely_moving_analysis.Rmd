...
bind_rows(
  ...
  dplyr::group_by(animal, body_part, intensity, condition_grp, stim_categ , stim_division) %>% # stim_number, , stim_duration
  # dplyr::group_by(animal, body_part, intensity, stim_side) %>% 
  dplyr::summarise(
    sum_d = sum(d, na.rm = T),
    mean_d = mean(d, na.rm = T),
  ...
  ungroup() %>%
  fill(max_sum_d, .direction = "down") %>% 
  arrange(animal, body_part, condition_grp, intensity) %>% 
  dplyr::filter(
    !is.na(condition_grp), 
    !str_detect(condition_grp, pattern = "bilat"),
