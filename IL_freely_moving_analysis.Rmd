...
ggplot(
  mapping = aes(
    x = forcats::fct_relevel(stim_categ, "before stim", "stim", "after stim"),
    y = sum_ang_ipsiconra/360 # sum_ang %>% abs() /360  # sum_ang %>% abs() # / max(sum_ang %>% abs()) # %>% scale()
  )
) +
  geom_point(aes(fill = condition_grp), shape = 21, size = 4) +
  geom_line(aes(group = condition_grp)) +
  ...
  annotate(geom = "text", x = 3, hjust = -0.15, y = -0.002, label = "ipsilateral", col = "gray") +
  annotate(geom = "text", x = 3, hjust = -0.15, y = 0.002, label = "contralateral", col = "gray") +
  expand_limits(y = c(-0.03, 0.03), x = c(0,4)) +
  # xlab("State") +
  # ylab("Scaled sum angle") +
  ggtitle("Stimulus") +
  theme(aspect.ratio = 1) +
