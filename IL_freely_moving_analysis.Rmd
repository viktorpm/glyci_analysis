...
coordsIL <- coords_rawIL %>%
  ...
  dplyr::mutate(
    condition_grp = dplyr::case_when(
      ### bilat
      animal == "GIIFM5" & stim_side %in% c("bilat IL", "bilat Pf") ~ "bilat",
      animal == "GIIFM6" & stim_side %in% c("bilat Pf") ~ "bilat",
      ### unilat left
...
```{r message=FALSE, fig.width=10, echo = FALSE}
sem <- function(x) sd(x) / sqrt(length(x)) # Create own function
...
ggplot(
  ...
  geom_point(aes(fill = stim_side), shape = 21, size = 4) +
  geom_line(aes(group = stim_side)) +
  ggpubr::stat_compare_means(
    method = "wilcox.test",
    paired = T,
    method.args = list(alternative = "less"),
...
bind_rows(
  ...
  dplyr::group_by(animal, body_part, intensity, stim_side, stim_categ , stim_division) %>% # stim_number, , stim_duration
  ...
  arrange(animal, body_part, stim_side, intensity) %>% 
  ggplot(
    mapping = aes(
