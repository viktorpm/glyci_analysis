...
coords <- read.csv(file.path("data", "turning", "GIIFM19.csv")) %>%
  ...
  dplyr::mutate(
    ...
    segments = cumsum(ifelse(rec_time %% 8 == 0, 1, 0))) %>% # length of the stimulus 
...
coords %>%
  ...
  ggplot(mapping = aes(x = rec_time, y = mps)) +
  geom_line(aes(color = stim_number, group = 1)) + 
...
```{r}
coords %>% 
  dplyr::group_by(stim_number) %>% 
  summarise(rec_time, mps) #%>% 
  #ggplot(mapping = aes(x = rec_time, y = mps, color = stim, group = 1)) + 
  # ylim(c(0,100)) +
  # geom_line()
