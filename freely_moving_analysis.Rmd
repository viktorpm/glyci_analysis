...
<!--     session = str_remove(session, ".dat"), -->
...
coords %>%
  dplyr::filter(animal == "GIIFM24b" ,body_part == "center") %>%
...
```{r fig.width=10, fig.height=10}
coords %>%
  dplyr::filter(animal == "GIIFM21b", body_part == "center") %>%
...
```{r fig.width=10, fig.height=10}
coords %>%
  dplyr::filter(animal == "GIIFM21b", body_part == "head") %>%
  ggplot(mapping = aes(x = rec_time, y = d_mm)) +
  geom_line(aes(color = stim, group = 1)) +
  # geom_line(aes(x = rec_time, y = Y_diff -130), alpha = .5) + 
  # geom_line(aes(x = rec_time, y = X_diff + 130), alpha = .5) + 
...
```{r fig.width=10, fig.height=10}
coords %>%
  dplyr::filter(animal == "GIIFM21b",body_part == "tail") %>%
  ggplot(mapping = aes(x = rec_time, y = d_mm)) +
  geom_line(aes(color = stim, group = 1)) +
  # geom_line(aes(x = rec_time, y = Y_diff -130), alpha = .5) + 
  # geom_line(aes(x = rec_time, y = X_diff + 130), alpha = .5) + 
...
coords %>%
  dplyr::filter(animal == "GIIFM21b", body_part == "center", session == "video2coords") %>%
  dplyr::group_by(stim) %>%
...
```{r message=FALSE, fig.width=10, fig.height=10}
coords %>%
  # dplyr::filter(animal == "GIIFM24b", body_part != "tail") %>%
  dplyr::filter(body_part != "tail") %>%
  ...
  geom_histogram(aes(col = session)) +
  facet_grid(animal ~session, scales = "free")
```
...
coords %>%
  ...
  add_column(
    rec_time = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(rec_time),
    stim = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(stim)
  ) %>%
...
```{r}
coords %>%
  dplyr::filter(animal == "GIIFM19", session == "video0coords", body_part != "tail") %>% 
  dplyr::group_by(body_part) %>%
  dplyr::slice(1:25) %>% 
  ungroup() %>% 
...
coords %>%
  ...
  add_column(
    rec_time = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(rec_time),
    stim = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(stim),
    stim_categ = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(stim_categ),
    stim_number = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(stim_number),
    Y_diff = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(Y_diff),
    X_diff = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(X_diff),
    d_mm = coords %>% dplyr::filter(animal == "GIIFM24b", body_part == "head") %>% pull(d_mm),
  ) %>%
  group_by(session) %>%
...
<!-- ```{r fig.show='animate', message=FALSE} -->
<!-- p_angle <- coords %>% -->
<!--   dplyr::filter(body_part != "tail") %>% -->
<!--   dplyr::group_by(session, frame) %>% -->
<!--   summarise( -->
