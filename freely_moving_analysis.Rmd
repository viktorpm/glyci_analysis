...
StrToID = function(x){
  num <- x %>% 
    ...
    unlist() %>% 
    paste(collapse = "") %>% 
    regmatches(gregexpr(text = .,"[[:digit:]]+")) %>% 
    unlist() %>% 
    paste(collapse = "") %>% 
    strsplit("") %>% 
    unlist() %>% 
  ...
  letter <- x %>% 
    ...
    unlist() %>% 
    paste(collapse = "") %>% 
    regmatches(gregexpr(text = .,"[[:alpha:]]+")) %>%
    unlist() %>% 
    paste(collapse = "")
...
coords <- read.csv(file.path("data", "PnO_stim_rotation", "rotation_all.csv")) %>%
  ...
  #   
  # ) %>% 
  # dplyr::filter(animal == "GIIFM21b", session == 2 ) %>%
  # View()
  ...
  # fill(Y) %>% 
  # ungroup() %>% 
  # dplyr::filter(animal == "GIIFM21b", session == 2) %>% View()
...
# lengths %>% dplyr::filter(animal != "GIIFM14b", stim_categ == "stim") %>% View()
```
...
Plotting traveled distance during individual stimuli (4 to 6 repeats) within session. Traveled distance is averaged through the full duration of the stimulus (~10s) and `r around_stim_length`s "before stimulus" and "after stimulus" duration. <br>
...
corrected_angles_population %>%
  ...
  facet_grid(animal ~ intensity, scales = "free") +
  scale_color_manual(values = group_colors) +
  global_facet_theme +
  theme(
...
Mean rotation angle by intensity (stimulus and control)
...
```{r message=FALSE, fig.height=10, fig.width=15, echo = FALSE}
plt1_dist <- coords %>%
  dplyr::filter(
    str_detect(string = animal, pattern = "b"),
    body_part == "center",
  ...
  geom_point(aes(color = intensity), position = position_jitterdodge(dodge.width = 0.7)) +
  #facet_wrap(~animal) +
  theme(legend.position = "none") +
  ylim(0,8) +
  xlab("Stimulus") +
