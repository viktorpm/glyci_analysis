...
stims %>%  dplyr::filter(animal == selected_animal) %>%  head(100)
```
...
coords %>% dplyr::filter(animal == selected_animal) %>% dplyr::group_by(session, body_part) %>% slice(1:5)
```
...
coords %>%
  ...
  dplyr::group_by(animal, body_part, session) %>%
  dplyr::mutate(
    X_diff_mean = mean(X_diff, na.rm = T),
    X_diff_range = paste0(range(X_diff, na.rm = T) %>% `[[`(1), ":", range(X_diff, na.rm = T) %>% `[[`(2)),
    X_diff_Q1 = quantile(X_diff, na.rm = T)["25%"],
    X_diff_Q3 = quantile(X_diff, na.rm = T)["75%"],
    X_diff_IQR = IQR(X_diff, na.rm = T),
    Y_diff_mean = mean(Y_diff, na.rm = T),
    Y_diff_range = paste0(range(Y_diff, na.rm = T) %>% `[[`(1), ":", range(Y_diff, na.rm = T) %>% `[[`(2)),
    Y_diff_Q1 = quantile(Y_diff, na.rm = T)["25%"],
    Y_diff_Q3 = quantile(Y_diff, na.rm = T)["75%"],
    Y_diff_IQR = IQR(Y_diff, na.rm = T),
  ) %>%
  summarise(
    X_diff_mean = X_diff_mean %>% unique(),
    X_diff_range = X_diff_range %>% unique(),
    X_diff_Q1 = X_diff_Q1 %>% unique(),
    X_diff_Q3 = X_diff_Q3 %>% unique(),
    X_diff_IQR = X_diff_IQR %>% unique(),
    Y_diff_mean = Y_diff_mean %>% unique(),
    Y_diff_range = Y_diff_range %>% unique(),
    Y_diff_Q1 = Y_diff_Q1 %>% unique(),
    Y_diff_Q3 = Y_diff_Q3 %>% unique(),
    Y_diff_IQR = Y_diff_IQR %>% unique(),
  )
```
...
<br>
Histograms of head-center distances with bloxplots and outliers.
<br>
...
<br>
Plotting first five head-center distances.
<br>
...
Plotting traveled distance during individual stimuli (4 to 6 repeats) within sessions. Traveled distance is averaged through the full duration of the stimulus (~10s) and `r around_stim_length / fps`s "before stimulus". Traveled distance "after stimulus" is not included.  <br>
...
coords %>%
  ...
  dplyr::summarise(mean_d_mm = sum(d_mm, na.rm = T)) %>%
  ggplot(
    mapping = aes(
...
## Mean traveled distance by individual stimuli within session (1s before and during the stimulus)


Plotting traveled distance during individual stimuli (4 to 6 repeats) within sessions. Traveled distance is averaged through the last second of "before stimulus" category and through first second of the stimulus. Traveled distance "after stimulus" is not included. <br>
...
<br>
The table shows the filtered data (1s before and 1s during stimulus)
<br>
...
Mean traveled distance across sessions at different intensities by animals
<br>
1 second before stimulus, first second of stimulation
...
Mean traveled distance across sessions at different intensities by animals (control)
<br>
1 second before stimulus, first second of stimulation
...
<br>
Traveled distance before stimulus vs during stimulus (starting or stopping signal). If the traveled distance is greater during the stimulus photoactivation initiates movement, if it is smaller it slows down the animal.
