...
selected_animal <- "GIIFM22b" # "GIIFM14b"#
...
coords %>%
  ...
  geom_label(data = text_layer, aes(x = 1000, y = 10, label = paste0("Intensity: ", int_label, " mW"))) +
  global_facet_theme
```
...
coords %>%
  ...
  ### "coords" was filtered differently
  geom_point(
    data = coords %>%
      dplyr::filter(
        animal == selected_animal,
...
coords %>%
  ...
  geom_label(data = text_layer, aes(x = 60, y = 15, label = paste0("Intensity: ", int_label, " mW"))) +
  global_facet_theme
```
...
coords %>%
  ...
  geom_label(data = text_layer, aes(x = 30, y = 150, label = paste0("Intensity: ", int_label, " mW"))) +
  global_facet_theme
```
...
```{r}
coords %>%
  dplyr::filter(animal == selected_animal, session == "video1coords", body_part != "tail") %>%
  dplyr::group_by(body_part) %>%
  dplyr::slice(1:25) %>%
  ungroup() %>%
...
coords %>%
  ...
  geom_line(aes(col = stim, group = 1)) +
  facet_wrap(~session, scales = "free_x", ncol = 2) +
  geom_label(data = text_layer, aes(x = 60, y = 2, label = paste0("Intensity: ", int_label, " mW"))) +
  global_facet_theme
```
...
```{r message=FALSE,fig.width=15, fig.height=13.5, out.width="100%"}
...
coords %>%
  ...
  facet_grid(session ~ animal, scales = "free") +
  global_facet_theme +
  theme(
...
coords %>%
  ...
  # !!!!!!!!!!!!!!!!!!!!!!!!!! check mutate  c(diff(X_interp), NA) vs diff(X_interp)
  dplyr::group_by(animal, session, frame) %>%
  mutate(
    vectx = diff(X_interp),
    vecty = diff(Y_interp),
...
coords %>%
  dplyr::filter(
    str_detect(string = animal, pattern = "b"),
    animal != "GIIFM14b",
    # intensity %in% c(0.1, 0.2, 0.5, 1), # intensity filter
    # intensity %in% c(2, 5, 6, 10, 15),
    body_part != "tail",
    stim_categ != "no stim"
  ) %>%
  dplyr::group_by(animal, session, frame) %>%
  mutate(
    vectx = c(diff(X_interp), NA),
    vecty = c(diff(Y_interp), NA)
  ) %>%
  group_by(animal, session) %>%
  mutate(
    leadx = lead(vectx),
...
Traveled distance before stimulus vs during stimulus (starting or stopping signal)


```{r fig.width=15, fig.height=13.5, out.width="100%"}
coords %>%
  dplyr::filter(
    str_detect(string = animal, pattern = "b"),
    body_part == "center",
