...
<br>
Length of stimuli, number of repeats of each stimulus with a given intensity
<br>
...
<br>
Histograms of head-center distances with boxplots and outliers.
<br>
...
plt1_angle <- corrected_angles_population %>%
  ...
  ggplot(
    mapping = aes(
      x = forcats::fct_relevel(stim_categ, "before stim", "stim", "after stim"),
      y = sum_ang %>% abs() # / max(sum_ang %>% abs()) # %>% scale()
    )
  ) +
  geom_point(aes(fill = animal), shape = 21, size = 4) +
  geom_line(aes(group = animal)) +
...
Sum traveled distance across sessions at different intensities by animals (stimulus and control)
<br>
...
plt_pop_dist_stim <- coords_1s_window %>%
  ...
  dplyr::mutate(
    ...
    ##these are calculated later in the code!!!! 
    # mean_sum_d = case_when(
    #   intensity == "1 mW" ~ mean_sum_d_1mW,
    #   intensity == "5 mW" ~ mean_sum_d_5mW,
    #   intensity == "10 mW" ~ mean_sum_d_10mW,
  ...
  ggplot(
    mapping = aes(
      x = forcats::fct_relevel(stim_categ, "before stim", "stim"),
      y = sum_d # / mean_sum_d #norm_ref # # norm_ref #
    )
  ) +
...
Sum traveled distance across sessions at different intensities by animals (stimulus and control)
<br>
8 second before stimulus, first 8 second of stimulation (8 because some "before stim" periods are only 8 seconds long)
...
Combining 1 sec before stim and stim division to 1st and last 9 sec tables. Needs work!!!
...
bind_rows(
  ...
  #     mps = (speed_instant * (1 / time_res)) / 1000
  #   ) %>%
  #   ## grouping by session vs intensity!!!!! check!!!! looks similar, sometimes the intensity and the session doesn't match
  ...
  coords %>%
    ...
    dplyr::mutate(
      stim_division = ifelse(rec_time <= min(rec_time) + 1, "first_part", "last_part"),
      stim_div_length = ifelse(rec_time <= min(rec_time) + 1, 1, 9)
    )
) %>% 
  ...
  dplyr::group_by(animal, intensity, stim_number, stim_division, stim_div_length) %>%
  dplyr::summarise(sum_d = sum(d, na.rm = T)) %>%
  dplyr::mutate(
    mean_sum_d = case_when(
      intensity == "1 mW" ~ mean_sum_d_1mW,
  ...
  ggplot(
    mapping = aes(
      ...
      y = (sum_d / stim_div_length)/mean_sum_d
    )
  ) +
  geom_point(aes(fill = stim_number %>% as.factor()), shape = 21, size = 4) +
  facet_grid(animal ~ factor(intensity, levels = gtools::mixedsort(intensity %>% unique()))) +
