...
coords %>%
  ...
  ungroup() %>%
  group_by(session) %>%
  mutate(stim_block_number = stim_number) %>% View()
...
```{r message=F, fig.width=15, fig.height=10, warning=FALSE}
corrected_angles %>%
  dplyr::filter(
    session == "session_3",
    body_part == "center",
...
norm_ref <- coords_1s_window %>%
  ...
  dplyr::group_by(animal, intensity, stim_categ) %>%
  dplyr::summarise(
    sum_d = sum(d, na.rm = T)) %>% pull(sum_d) %>% max()
...
Sum traveled distance across sessions at different intensities by animals (stimulus and control)
<br>
8 second before stimulus, first 8 second of stimulation
