...
+ **text_layer**: stimulus intensity in each session (used to create the text layer on plots)
...
coords %>%
  ...
  geom_text(data = text_layer, aes(x = 1000, y = 10, label = paste0("Intensity: ", int_label, " mW")))+
  global_facet_theme
```
...
```{r message=F, fig.width=20, fig.height=10}
coords %>%
  dplyr::filter(animal == selected_animal, body_part != "tail") %>%
  ...
  geom_text(data = text_layer, aes(x = 30, y = 150, label = paste0("Intensity: ", int_label, " mW")))+
  global_facet_theme
...
coords %>%
  ...
  geom_line(aes(col = stim, group = 1)) +
  facet_wrap(~session, scales = "free_x") +
  geom_text(data = text_layer, aes(x = 60, y = 2, label = paste0("Intensity: ", int_label, " mW")))+
  global_facet_theme
```
