...
coords %>% dplyr::filter(animal == selected_animal) %>%  head(100)
```
...
lengths <- coords %>%
  ...
  count(stim_categ) %>% 
  spread(stim_categ, n) %>% 
  select(animal, session, stim_number, `before stim`, stim, `after stim`) 
...
coords %>% 
  ...
  geom_histogram() +
  geom_boxplot(aes(y = 1000), width=250, outlier.colour = "magenta", col = "blue" ) +
