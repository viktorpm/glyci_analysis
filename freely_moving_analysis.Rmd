...
```{r}
cowplot::plot_grid(plt_taject_stim_baseline, plt_taject_stim_stim, plt_taject_ctrl_baseline, plt_taject_ctrl_stim, nrow = 2)
```

```{r}
cowplot::plot_grid(plt_taject_stim_wrap, plt_taject_ctrl_wrap, nrow = 2)
```
...
ggplot(
  ...
  geom_point(aes(fill = intensity), shape = 21, size = 4) +
  geom_line(aes(group = intensity)) +
  ggpubr::stat_compare_means(
    method = "wilcox.test",
    paired = T,
...
bind_rows(
  ...
  dplyr::group_by(intensity, stim_categ, stim_division, across(stim_div_length)) %>% # stim_number,
  dplyr::summarise(
    sum_d = sum(d, na.rm = T),
    mean_d = mean(d, na.rm = T),
    max_sum_d = sum_d %>% max()
  ...
  theme(
    ...
    panel.background = element_blank(),
    axis.line = element_line(colour = "black")
  ) -> plt_intensity_dose_stim #plt_intensity_dose_ctrl


cowplot::plot_grid(plt_intensity_dose_stim, plt_intensity_dose_ctrl, ncol = 2)
...
```{r fig.width=15}
 coords %>%
  ...
  ggplot(
    aes(x = forcats::fct_relevel(intensity, "1 mW", "5 mW", "10 mW", "15 mW"), y = mean_d)
  ) + 
  geom_point(aes(fill = animal), shape = 21, size = 4) +
  geom_boxplot(alpha = 0) +
  expand_limits(y = c(0, 60)) +
