...
selected_animal <- "GIIFM36b" # "GIIFM14b"#
...
```{r, message=F, warning=F,  fig.width=10,fig.height=25, message=FALSE}
coords %>%
...
```{r message=F, fig.width=10, warning=F}
coords %>%
  dplyr::filter(
    animal == selected_animal,
    body_part == "center",
...
coords %>%
  ...
  dplyr::group_by(body_part) %>%
  slice(1:3) %>%
  dplyr::filter(body_part != "tail", frame == 0) %>% pull(X)
```
...
mutate(signed_angle_corrected = ifelse(signed_angle_corrected %>% abs() > 100, 0, signed_angle_corrected))
...
coords %>%
  ...
  add_column(
    ...
    intensity = coords %>% dplyr::filter(animal == selected_animal, body_part == "head") %>% pull(intensity)
  ) %>%
  group_by(session) %>%
...
coords %>%
  ...
  dplyr::group_by(animal, intensity, frame) %>%
  mutate(
...
```{r message=FALSE, warning=FALSE, fig.width=17, fig.height=15, out.width="100%"}
coords %>%
  dplyr::filter(
    str_detect(string = animal, pattern = "b"),
    body_part == "center",
