...
coords <- read.csv(file.path("data", "PnO_stim_rotation", "rotation_all.csv")) %>%
  ...
  fill(intensity) %>%
  fill(stim_freq) %>%
  select(-grp, -timing, -stim_length) %>% # , -animal
  ungroup() %>%
...
coords %>%
 ...
 dplyr::filter(body_part == "head") %>%
  group_by(session, intensity, body_part) %>%
  mutate(csum = cumsum(ifelse(is.na(signed_angle_corrected), 0, signed_angle_corrected)) %>% round(4)) %>%
 
  ggplot(
    mapping = aes(x = rec_time, y = csum %>% scale())
  ) +
  geom_line(aes(col = stim_categ), alpha = 0.5) +
  facet_grid(session~intensity, scales = "free_x")
```
...
coords %>%
  ...
  group_by(animal, session, intensity) %>%
  mutate(csum = cumsum(ifelse(is.na(signed_angle_corrected), 0, signed_angle_corrected)) %>% round(4)) %>%
  ...
  facet_wrap(animal~session, scales = "free") 
```
...
coords %>%
  ...
  geom_point(aes(fill = animal), shape = 21, size = 4, alpha = .5) +
