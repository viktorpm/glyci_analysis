...
```{r}
vectors <- tibble(
  body = c("cent", "h", "cent", "h", "cent", "h"),
  frame = c(0, 0, 1, 1, 2, 2),
  X = c(750, 760, 770, 770, 755, 765),
  Y = c(500, 500, 500, 520, 510, 520)
)
...
coords %>%
  ...
  geom_text_repel(aes(label = paste0(X, ",", Y),col = as.character(frame))) +
  geom_segment(aes(x = 210 , xend = 255,
               y = 345, yend = 345), linetype = "dashed", col = "red") +
  geom_segment(aes(x = 255 , xend = 255,
               y = 345, yend = 364), linetype = "dashed", col = "red")
...
coords %>%
  ...
  add_column(
    ...
    before_stim = coords %>% dplyr::filter(body_part == "head") %>% pull(before_stim),
    stim_number = coords %>% dplyr::filter(body_part == "head") %>% pull(stim_number)
  ) %>% 
  gather(key = "stim_categ" , value = "stim_value", stim, before_stim) %>% 
  ggplot(
    mapping = aes(x = rec_time, cumsum(ifelse(is.na(signed_angle), 0, signed_angle) %>% scale()  ))
  ) + 
    geom_line(aes(col = stim_categ, linetype = session)) +
