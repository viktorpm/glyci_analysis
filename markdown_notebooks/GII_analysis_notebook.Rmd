...
```
```{r setup, include=FALSE, tidy=TRUE, tidy.opts=list(width.cutoff=50)}
library(R.matlab)
library(tidyverse)
library(reshape2)
library(ggrepel)
...
CELL_INFO <- info_baseline %>%
  ...
  dplyr::mutate(
    gap = ifelse(cell_id %in% c("cell06", "cell07", "cell08", "cell09"), yes = "y", no = "n")
  )
...
<br> **sd_mean_isi**: Calculating the baseline MFR of the recorded IL cells from the *IL_baseline_firing* table using a custom made function (*SDMeanISI*). The results are stored in the *sd_mean_isi* table.
...
$$\mathbf{activity\_change} = \frac{during\_MFR - base\_MFR}{base\_MFR} * 100$$ <br>
...
$$\mathbf{activity\_change} = \frac{during\_MFR - before\_MFR}{before\_MFR} * 100$$ <br>
...
Comparison of baseline and "before" stimulus firing rates in the case of spontaneously active and spontaneously inactive (pinch) neurons. Spontaneously inactive neurons showed significantly higher MFR before stimulus compared to baseline.

Categorizing cells into week and strong based on the dynamics of inhibition
...
psth_tibble_gap %>%
  ...
  # dplyr::filter(cell_id %in% c("cell06","cell07","cell08","cell09","cell11","cell16")) %>%
  ggplot(
    aes(x = reltimes)
  ) +
...
# plt_pinch_nopinch_to_baseline$data %>% 
#   dplyr::group_by(pinch, stim_cond) %>% 
#   dplyr::summarise(
#     mean(MFR),
#     sd(MFR),
#     median(MFR)
...
# MFR_plot$data %>% 
#   dplyr::filter(pinch == T) %>% 
#   dplyr::group_by(stim_cond) %>% 
#   dplyr::summarise(mean(MFR), sd(MFR), median(MFR))
# 
# MFR_plot$data %>% 
#   dplyr::filter(pinch == F) %>% 
#   dplyr::group_by(stim_cond) %>% 
#   dplyr::summarise(mean(MFR), sd(MFR), median(MFR))
...
<br><br> Normalized to "before" value <br><br>
...
ggplot(
  ...
  # stroke = 2) +
  geom_line(
    aes(group = cell_id, col = position)
  ) +
  geom_label_repel(
    data = TO_PLOT %>%
      dplyr::filter(
...
### CAREFUL WITH THIS!! It's generated by the "CreatePSTHTibble" function but the frequencies have to be changed manually, and it can create duplicates! I have to work on that! "CreatePSTHTibble" is defined in "cortical_stim_analysis.R" file. A warning is written there too.
...
datatable(
  RECORDINGS %>%
    dplyr::group_by(signal_type, stim_freq) %>%
    slice(1:3),
...
Stimuli at 8, 10 and 12 Hz and 18 and 20 Hz were grouped.
...
GV_40, GV_42 and GV_47 are electrical stimulations. Might need to be excluded from analysis as well as GII_20 (looks like an outlier).   \* Response probability was calculated two different ways: + as the number of APs in the boxplot range (25th percentile - 1.5\*IQR, 75th percentile + 1.5\*IQR) \* 100 (if the lower range was smaller than 0, values were cut at zero) + between 5-20 ms after the stimulus

<br><br><br>

$$\mathbf{Response\ probability} = \frac{Number\ of\ APs}{Number\ of\ stimuli} * 100$$

<br><br><br>

Response probability <br><br>
...
### mean, median, sd
# RESP_PROB %>%
#   dplyr::group_by(stim_freq) %>%
#   dplyr::summarise(
#     mean(resp_prob_range),
#     median(resp_prob_range),
#     sd(resp_prob_range))
```

<br><br> Number of APs <br><br>
...
APC %>%
  ...
  dplyr::group_by(animal_id, stim_freq_categ) %>%
  summarise(No_APs = mean(No_APs)) %>%
  ggplot(
    aes(x = stim_freq_categ, y = No_APs)
  ) +
  geom_boxplot(alpha = 0, width = 0.2, lwd = 1, fatten = 1.2) +
  geom_point(
    aes(
...
### Number of APs after each stimulus in stimulus trains (50 ms window). Normalized by the maximum number of AP followed by a stimulus in each animal.

<br><br><br>

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.align='center'}
# Normalized!!!! Check APC
ggplot(
  data = APC %>%
    dplyr::filter(
      stim_freq_categ %in% c("1", "10", "20"),
    ...
    ungroup() %>%
    dplyr::group_by(animal_id) %>%
    mutate(sumnorm_AP = sum_No_APs / max(sum_No_APs), maxAP = max(sum_No_APs)),
  mapping = aes(
    x = stim_number,
    y = sumnorm_AP,
  ...
  # xlim(1,10) +
  facet_wrap(~stim_freq_categ, ncol = 1) +
...
#     ) %>%
#   dplyr::group_by(ID) %>%
#   summarise(No_desync_periods = length(state_number %>% unique()))

```

### Wavelet

```{r echo=FALSE, fig.width=15, fig.height=10, message=FALSE, warning=FALSE}
...
bind_rows(sync_reltimes, desync_reltimes) %>%
  ...
  ggh4x::force_panelsizes(
    cols = unit(c(4, 4), "inch"),
    rows = unit(c(3, 3, 3, 3, 3, 3, 3), "inch"), TRUE
  ) +
  global_facet_theme +
  theme(
    axis.title.x = element_text(size = 12),
...
#   x = EEG_ds_df %>%
#   dplyr::filter(!is.na(eeg_state), state_length >= 1 / first_peak) %>%
#   dplyr::group_by(ID, eeg_state, state_number) %>%
#   dplyr::summarise(
#     sum1 = sum(state_length %>% unique(), na.rm = T),
#     No_clusters = sum(burst_isi, na.rm = T)) %>%
#   dplyr::group_by(ID, eeg_state) %>%
...
#   pull(No_clusters)
#
# wilcox.test(x=x, y=y, alternative = "less", paired = T)
...
# plt_clust_rate_state_wrapped$data %>% 
#   dplyr::group_by(animal,eeg_state) %>% 
#   dplyr::summarise(
#     mean( No_clusters / sum1),
#     sd( No_clusters / sum1),
#     median( No_clusters / sum1)
#   )
...
```{r}
plt_clust_rate_animal$data %>%
  dplyr::filter(animal %!in% c("GII21", "GV115")) %>%
  dplyr::group_by(eeg_state) %>% 
  dplyr::summarise(
    mean(No_clusters / state_length_sum), 
    sd(No_clusters / state_length_sum), 
    median(No_clusters / state_length_sum))


plt_clust_rate_animal$data %>%
  dplyr::filter(animal %!in% c("GII21", "GV115")) %>%
  dplyr::filter(eeg_state == "desync") %>%
  dplyr::mutate(rx = No_clusters / state_length_sum) %>%
  pull(rx) %>% sd()
...
plt_clust_rate_animal$data %>%
  dplyr::filter(animal %!in% c("GII21", "GV115")) %>%
  dplyr::filter(eeg_state == "sync") %>%
  dplyr::mutate(ry = No_clusters / state_length_sum) %>%
  pull(ry) %>% sd()

rx <- function(n) rnorm(n, mean = 0.6354203, sd = 0.439563) 
ry <- function(n) rnorm(n, mean = 1.327837, sd = 0.574624) 

MKpower::sim.power.wilcox.test(nx = 5, rx = rx, ny = 5, ry = ry, alternative = "less")
MKpower::sim.ssize.wilcox.test(rx = rx, ry = ry, n.min = 5, iter = 1000,  alternative = "less")
...
EEG_ds_df %>%
  ...
  # dplyr::group_by(animal, eeg_state) %>%
  # summarise(state_length_sum = sum(sum1), No_clusters = sum(No_clusters)) %>%
  ggplot(aes(x = eeg_state, y = No_AP / sum1)) + # state_length_sum)) +
  geom_jitter(
    data = . %>% dplyr::filter(!grepl(pattern = "GV120", animal)),
    aes(col = animal), size = 3, width = 0.1
  ) +
  ...
  geom_boxplot(
    data = . %>% dplyr::filter(!grepl(pattern = "GV120", animal)),
    aes(x = eeg_state, y = No_AP / sum1), # state_length_sum),
    inherit.aes = F,
    alpha = 0, width = 0.2, lwd = 1, fatten = 1.2
  ) +
  ggpubr::stat_compare_means(
...
### Number of clusters before and during cortical inactivation
...
bind_rows(baseline_reltimes, csd_reltimes) %>%
  ...
  ggh4x::force_panelsizes(
    cols = unit(c(4, 4), "inch"),
    rows = unit(c(3, 3, 3, 3, 3), "inch"), TRUE
  ) +
  global_facet_theme +
  theme(
    axis.title.x = element_text(size = 12),
...
TERMINALS %>%
  dplyr::filter(postion != "non_labeled") %>%
  ggplot(
...
```{r echo=FALSE, fig.width=10, message=FALSE, warning=FALSE}
TERMINALS %>%
  ggplot(mapping = aes(x = postion, y = d_dendrite)) +
  geom_half_violin(
    position = position_nudge(x = -.15),
    trim = F,
    side = "l",
  ...
  geom_jitter(position = position_jitter(0.1)) +
  geom_signif(
    comparisons = list(c("glyt2", "non_labeled")),
    map_signif_level = TRUE,
    test = "mood.test",
    margin_top = .1,
    color = "gray"
