...
### !!!!!!!!!! check it. loks like the images are in portrai orientation !!!!!!
...
```{r}
plt_taject_stim_wrap <- coordsIL %>%
  dplyr::filter(
    animal == selected_animal,
    intensity == "10 mW",
    frame %% datapoint_to_plot == 0,
    condition_grp == "GIIFM6 left Pf",
    session == "video25coords.dat.19fix.bak"
  ) %>% 
  ggplot(
    mapping = aes(
      x = X_interp, # / scale,
...
cowplot::plot_grid(
  plt1_angle, plt1_angle_categ_length_norm,
  nrow = 2,
  labels = "IL stimulus freely moving experiments"
)
```
...
```{r fig.width=15, fig.height=5}
  ...
  ggtitle(paste0(title, ", ", "traveled distance before, during and after stimulus (in 1 sec long windows)"))
