...
Run this only once to load raw data, clean it from junk and save it again to reduce file size for GitHub. Original raw data is not version controlled. Cleaned data is loaded for further analysis
...
stims <- read.csv(file.path("data", "PnO_stim_rotation", "rotation_stims_cleaned.csv")) %>%
  as_tibble() %>%
  mutate(
...
coords_raw <- read.csv(file.path("data", "PnO_stim_rotation", "rotation_all_cleaned.csv")) %>%
  as_tibble() %>%
  mutate(
...
coords <- coords_raw %>%
  ...
  ungroup() %>%
  ### adding the number of repeats in each intensity group, (no -1 as "intensity" variable doesn't run through the session)
...
```{r}
cowplot::plot_grid(plt_taject_stim_wrap, plt_taject_ctrl_wrap, nrow = 1)
```
...
```{r message=FALSE, fig.height=5, fig.width=20, echo = FALSE}
sem <- function(x) sd(x) / sqrt(length(x)) # Create own function
...
corrected_angles_population %>%
  ...
  ggplot(
    mapping = aes(
      x = forcats::fct_relevel(stim_categ, "before stim", "stim", "after stim"),
      y = sum_ang_ipsicontra / 360 # sum_ang %>% abs() /360  # sum_ang %>% abs() # / max(sum_ang %>% abs()) # %>% scale()
    )
  ) +
  geom_point(aes(fill = animal), shape = 21, size = 4) +
  geom_line(aes(group = animal)) +
  ggpubr::stat_compare_means(
    data = . %>% dplyr::filter(ipsicontra == "contra"),
    method = "kruskal.test",
    paired = T,
    # method.args = list(alternative = c("greater")),
    # ref.group = "stim",

    comparisons = list(c("before stim", "stim"), c("stim", "after stim")),
    label.y = 30,
    hide.ns = F
    # label.y = 3000
  ) +
  ggpubr::stat_compare_means(
    data = . %>% dplyr::filter(ipsicontra == "ipsi"),
    method = "kruskal.test",
    paired = T,
    # method.args = list(alternative = c("greater")),
    # ref.group = "stim",
  ...
  annotate(geom = "text", x = 3, hjust = -0.15, y = -2, label = "ipsilateral", col = "gray") +
  annotate(geom = "text", x = 3, hjust = -0.15, y = 2, label = "contralateral", col = "gray") +
  global_facet_theme +
  expand_limits(y = c(-30, 20), x = c(0, 4)) +
  # xlab("State") +
  # ylab("Scaled sum angle") +
  ggtitle("Stimulus") +
  theme(aspect.ratio = 1) +
...
cowplot::plot_grid(plt_ang_dose_stim, plt_ang_dose_ctrl, ncol = 2)



# plt_ang_dose_ctrl$data %>%
...
bind_rows(
  ...
  #
  # dplyr::group_by(animal, session, stim_number, stim_division) %>%
  # ### length of each stim_division by session
  # ### length of each stim_division in seconds by session
  # dplyr::mutate(
  #   stim_div_length_row = length(stim_division),
  #   stim_div_length_sec = rec_time[length(rec_time)] - rec_time[1]
  #   ) %>%
  # ungroup() %>%
  # dplyr::group_by(animal, intensity, stim_number, stim_division) %>%
  # dplyr::mutate(
  #   stim_div_length_row_intensity = length(stim_division),
  #   #stim_div_length_sec_intensity
  #   ) %>%
  # ungroup() %>%
  ...
  # ### length of all the stim_division in one session in rows (5 repeats x 6 row before_stim)
  # dplyr::group_by(animal, session, stim_division) %>% #intensity
  # dplyr::mutate(
  #     all_stim_div_length_row=length(frame),
  #     all_stim_div_length_sec = stim_div_length_sec[1] * No_repeats
  #   ) %>%
  # ungroup() %>%

  # dplyr::group_by(animal, intensity, stim_division) %>%
  # dplyr::mutate(
  #     all_stim_div_length_row_intensity=length(frame)
  #   ) %>%
  # ungroup() %>%
  #
  #
  # dplyr::group_by(
  #   animal, intensity, stim_categ, stim_division,
  #   across(stim_div_length_row),
  #   across(stim_div_length_sec),
  #   across(all_stim_div_length_row),
  #   across(all_stim_div_length_sec),
  #   across(stim_div_length_row_intensity),
  #   across(all_stim_div_length_row_intensity),
  #
  #   ) %>% # stim_number,
  ...
  ungroup() %>%
  fill(max_sum_d, .direction = "down") %>% # write.csv(file = file.path("output_data","traveled_distance_source.csv"))
  ### plotting
  ggplot(
    mapping = aes(
...
# plt_pop_dist_stim$data %>%
#   dplyr::group_by(intensity,stim_division) %>%
#   dplyr::summarise(mean(mean_d * 6/scale), sd(mean_d * 6/scale), median(mean_d * 6/scale))
#
#
#
# plt_pop_dist_ctrl$data %>%
#   dplyr::group_by(intensity,stim_division) %>%
#   dplyr::summarise(mean(mean_d * 6/scale), sd(mean_d * 6/scale), median(mean_d * 6/scale))
```
...
cowplot::plot_grid(plt_dose_stim, plt_dose_ctrl, ncol = 2)
