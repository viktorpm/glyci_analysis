...
title = "Pno stimulus freely moving analysis"
```
...
generate_angle_stim_plot <- function(include_control) {
  corrected_angles_population %>%
    dplyr::filter(
      if (include_control) {
...
plt_ang_ctrl <- generate_angle_stim_plot(TRUE)
plt_ang_stim <- generate_angle_stim_plot(FALSE)
...
generate_pop_dist_1s_plot <- function(include_control) {
  coords_1s_window %>%
    dplyr::filter(
      str_detect(string = animal, pattern = "b"),
      body_part == "center",
...
plt_pop_dist_1s_ctrl <- generate_pop_dist_1s_plot(TRUE)
plt_pop_dist_1s_stim <- generate_pop_dist_1s_plot(FALSE)


# sum_trav_dist_norm_by_before_mean
cowplot::plot_grid(plt_pop_dist_1s_ctrl, plt_pop_dist_1s_stim, ncol = 2)
```
...
generate_pop_dist_8s_plot <- function(include_control) {
  coords_8s_window %>%
    dplyr::filter(
      str_detect(string = animal, pattern = "b"),
      body_part == "center",
...
plt_pop_dist_8s_ctrl <- generate_pop_dist_8s_plot(TRUE)
plt_pop_dist_8s_stim <- generate_pop_dist_8s_plot(FALSE)

cowplot::plot_grid(plt_pop_dist_8s_stim, plt_pop_dist_8s_ctrl, ncol = 2)
```
...
```{r fig.width=15}
generate_pop_dist_bda_summary_plot <- function(include_control) {
  coords_1s_window %>%
  dplyr::filter(body_part == "center") %>% 
  dplyr::group_by(animal, intensity, stim_categ) %>%
  dplyr::summarise(
...
plt_pop_dist_bda_sum_stim <- generate_pop_dist_bda_summary_plot(FALSE)
plt_pop_dist_bda_sum_ctrl <- generate_pop_dist_bda_summary_plot(TRUE)

cowplot::plot_grid(
  plt_pop_dist_bda_sum_stim, plt_pop_dist_bda_sum_ctrl,
  nrow = 2,
  labels = paste0(title, ", " , "traveled distance before, during and after stimulus (in 1 sec long windows)"),
  label_size = 11
  )
