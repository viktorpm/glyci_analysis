...
coords_halo %>%
  ...
  ggtitle(paste0(SELECTED_ANIMAL_GRP, " instantaneous traveled distance ", "(center)")) +
  global_facet_theme
```
...
coords_halo %>%
  ...
  geom_label(data = text_layer, aes(x = -Inf, y = Inf, hjust = -.2, vjust = 1.3, label = paste0("Intensity: ", int_label))) +
  facet_wrap(~session, scales = "free", ncol = 1) +
  ggtitle(paste0(SELECTED_ANIMAL_GRP, " instantaneous traveled distance ", "(head)")) +
  global_facet_theme
```
...
coords_halo %>%
  ...
  facet_wrap(~session, scales = "free", ncol = 1) +
  geom_label(data = text_layer, aes(x = -Inf, y = Inf, hjust = -.2, vjust = 1.3, label = paste0("Intensity: ", int_label))) +
  ggtitle(paste0(SELECTED_ANIMAL_GRP, " mean traveled distance by stimulus period ", "(head)")) +
  scale_color_manual(values = group_colors) +
  global_facet_theme
```
...
coords_halo %>%
  ...
  ggtitle(paste0(SELECTED_ANIMAL_GRP, " sum traveled distance by individual stimuli within session ", "(center)")) +
  labs(
    fill = "Intensity",
    x = "Stimulus period",
...
corrected_angles %>%
  ...
  geom_label(data = text_layer, aes(x = -Inf, y = Inf, hjust = -.2, vjust = 1.3, label = paste0("Intensity: ", int_label))) +
  ggtitle(paste0(SELECTED_ANIMAL_GRP, " raw angles by stimulus period ")) +
  global_facet_theme
```
...
corrected_angles %>%
  ...
  facet_wrap(~session, scales = "free_x", ncol = 1) +
  geom_label(data = text_layer, aes(x = -Inf, y = Inf, hjust = -.2, vjust = 1.3, label = paste0("Intensity: ", int_label))) +
  ggtitle(paste0(SELECTED_ANIMAL_GRP, " cumulative angles by stimulus period ")) +
  global_facet_theme
```
...
corrected_angles_population %>%
  ...
  facet_grid(animal_grp ~ session) + # factor(intensity, levels = gtools::mixedsort(intensity %>% unique())), scales = "free") +
  scale_color_manual(values = group_colors) +
  global_facet_theme +
  theme(
...
corrected_angles_population %>%
  ...
  dplyr::group_by(animal_grp, intensity, stim_number) %>%
  dplyr::summarise(
    align_time = rec_time - rec_time[1],
    cumang = cumsum(signed_angle_corrected) %>% scale(),
    across(stim_categ)
  ...
  ggplot(aes(x = align_time, y = cumang)) +
  geom_line(aes(group = stim_number, col = stim_categ)) +
  facet_grid(animal_grp ~ intensity, scales = "free_x")
...
corrected_angles_population %>%
  ...
  dplyr::group_by(animal_grp, stim_categ, stim_side) %>%
  dplyr::summarise(
    sum_ang = sum(signed_angle_corrected, na.rm = T),
    mean_ang = mean(signed_angle_corrected, na.rm = T),
    med_ang = median(signed_angle_corrected, na.rm = T),
...
generate_angle_stim_plot <- function(stimside) {
  corrected_angles_population %>%
    ...
    dplyr::group_by(animal_grp, intensity, stim_categ, stim_number) %>%
    dplyr::summarise(
      sum_ang = sum(signed_angle_corrected, na.rm = T),
      # mean_ang = mean(signed_angle_corrected, na.rm = T),
      # med_ang = median(signed_angle_corrected, na.rm = T),
...
coords_halo_1s_window %>%
  ...
  facet_wrap(~animal_grp, scales = "free")
```
