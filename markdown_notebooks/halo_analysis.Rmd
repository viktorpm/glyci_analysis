...
cowplot::plot_grid(plt_ang_dose_right, plt_ang_dose_left, ncol = 2)
```
...
norm_ref <- coords_halo_1s_window %>%
  dplyr::filter(
    body_part == "center",
    stim_categ != "no stim",
...
```{r message=FALSE, fig.height=15, fig.width=17, echo = FALSE}
norm_ref <- coords_halo_8s_window %>%
  dplyr::filter(
    body_part == "center",
    stim_categ != "no stim",
    intensity %in% c(selected_intensities),
...
stim_bstim2 <- coords_halo_1s_window %>%
  dplyr::filter(
    body_part == "center",
    animal %in% control_animals,
    stim_categ != "no stim",
